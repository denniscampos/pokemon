...
function getPokemon() {
  ...
  fetch(url)
    ...
    .then((data) => {
      ...
      document.querySelector("#baseExperience").innerText =
        data.base_experience;

      // Pokemon Ability #1
      document.querySelector(
        "#abilityOne"
      ).innerText = data.abilities[0].ability.name.toUpperCase();

      // Pokemon Ability #2
      document.querySelector(
        "#abilityTwo"
      ).innerText = data.abilities[1].ability.name.toUpperCase();

      // Pokemon official artwork
      document.querySelector(".official_artwork").src =
